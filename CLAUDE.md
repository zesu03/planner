...
2. **Whole-object lane (debounced, gated)** — `goals` and `qaza` only (deeply structured; don't map to field sentinels). Debounced `setDoc(..., { merge: true })` (`WRITE_DEBOUNCE_MS`, 500ms). A `loadedRef` gates `save()` (and `muhasabaLoadedRef` / `focusLoadedRef` gate the subcollection flushes) — **no write fires until a SERVER snapshot returns** (`gateOpenForDoc`/`gateOpenForCollection` open only on `fromCache === false`). This still prevents the whole-object clobber (empty-initial or stale-cache flush over newer server data — the recurring account-wipe). `dirtyRef` + `applyField`'s dirty-skip protect these two fields both directions (a flush sends only edited fields; an incoming snapshot skips a field with an unflushed local edit).
...
**When adding a new top-level field:** if it's additive/high-churn (arrays, counters, independent sub-fields), put it on the **delta lane** (a `*Delta` reducer in `lib/sync.js` + `writeDelta`, no `dirtyRef`). Only use the whole-object lane (state + `latest*Ref` + `dirtyRef` flag + a line in `buildDirtyPayload`) for a genuinely structured field like `goals`; for unbounded growth prefer the muhasaba subcollection pattern. The residual limitation on the whole-object lane is same-field simultaneous edits (last-writer-wins).
