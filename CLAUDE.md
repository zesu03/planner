...
The hook subscribes via `onSnapshot` (one listener on the user doc, plus subcollection listeners for `muhasaba` and `focusLog`), exposes eight `update*` setters (one per app-level field), and writes via a **debounced** `setDoc(..., { merge: true })` (`WRITE_DEBOUNCE_MS`, currently 500ms). Because writes are debounced, the hook keeps `latest*Ref` mirrors so rapid updates don't lose data. A `loadedRef` gates `save()` (and `muhasabaLoadedRef` / `focusLoadedRef` gate the subcollection flushes) — **no write fires until a SERVER snapshot returns** (`gateOpenForDoc`/`gateOpenForCollection` in `lib/sync.js` open only on `fromCache === false`; a cached snapshot — hit OR miss — never opens the gate). This prevents two data-wipe modes: (1) a setter firing before Firestore responds flushing empty initial refs over real data (once wiped a doc when geolocation resolved mid-load); and (2) a device holding a **stale offline IndexedDB cache** flushing that old data (a tap, or the qaza reconcile pass) over newer server data before the server snapshot arrives — the recurring account-wipe fixed by requiring server confirmation. Trade-off: the first edit of a session can't persist until the server is heard from once (reads still render from cache immediately; `loading` flips false on any snapshot, `loaded` only on the server one).
