...
src/
  ...
  hooks/               per-domain custom hooks (extracted from Planner)
    ...
    useFocusTimer.js   dial state, the 1-second tick interval, end-of-session
                       bookkeeping (writes a focusLog entry + chime; task
                       minute/session totals derive from focusLog, so nothing
                       is written back onto the goal), reset/end-early
                       semantics, pom-duration persistence. Takes `focusLog` to
                       compute the draining ETA budget (per-DAY for habits,
                       lifetime for one-shots)
...
- `goals` — **sharded to the `users/{uid}/goals/{goalId}` subcollection** (one doc per goal), no longer an inline field. `useUserData` still exposes it as the same array and `updateGoals` still takes a whole-array updater — the hook diffs by goal `id` (reusing `diffFocusIds`) to write/delete just the changed goal docs, on its own debounce timer + load gate (`goalsLoadedRef` / `pendingGoalIdsRef` / `goalsTimerRef`). Each goal `{ id, createdAt, title, type, category, due, notes, intention, completedAt, tasks[] }`. **`createdAt` (epoch ms) is the sort key** — subcollection docs load unordered, so `reconcileGoalsSnapshot` sorts **oldest-first** (ascending, unlike focusLog's newest-first) to preserve insertion order for the Muhasaba goal-check list + sort tiebreaks; set by `addGoal` and back-stamped on migration (`stampGoalsForMigration`, ascending `base+i`). Same seed→write→clear-inline migration as focusLog (`goalsMigratedRef`), gated on a SERVER snapshot. Per-goal docs mean concurrent edits to *different* goals no longer clobber, and the 1 MB user-doc limit no longer applies. **Two task flavours in the same `tasks[]` array** (see helpers in [src/lib/goals.js](src/lib/goals.js)):
   - **One-shot task**: `{ id, text, priority, eta, done }`. Standard task — flips `done` once, gates goal completion via the auto-complete check ("all one-shots done → set completedAt"). **A task's logged minutes and session count are NOT stored on the task** — they derive from `focusLog` (the single source of truth) via `focusStatsByTask` / `taskLoggedMins` in [src/lib/goalStats.js](src/lib/goalStats.js). (Legacy `sessions`/`totalTime` fields may linger on old docs; they're ignored.) So deleting a focusLog entry reverses its credit automatically — no counter to adjust — and the "Logged" tile can't drift from the rhythm sparkline.
   - **Recurring task (habit)**: `{ id, text, priority, eta, recurring: { type: "daily" | "weekly", days?: [0..6] }, completions: ["YYYY-MM-DD", ...] }`. Doesn't have a permanent `done`; today's tick comes from `completions.includes(todayStr())`. Weekly tasks use JS day-of-week (0=Sun…6=Sat); the classical Sunnah-fasting cadence is `{ type: "weekly", days: [1, 4] }`. Habits never gate goal completion and don't count in `pct()` — they're tracked via per-task streak and 30-day completion rate instead (habit-only goals show `habitGoalRate` — mean 30-day consistency — in the progress ring/bar in place of a permanent 0%).
...
- `settings: { prayerCity, prayerCountry, prayerLat, prayerLng, prayerMethod, prayerSchool, jamaahTimes, pomDurations, theme, dailyFocusGoalMins, qazaDailyTarget }` (`qazaDailyTarget` — makeups/day, drives the qaza completion projection; default 5. `prayerMethod`/`prayerSchool` — Aladhan calc method + Asr madhab, default `2`/`1` = ISNA + Hanafi; see External APIs. `jamaahTimes` — optional per-prayer mosque congregation times `{ [Prayer]: "HH:MM" }`; where set they override the Aladhan start as the moment the focus timer counts down to — see [src/lib/jamaah.js](src/lib/jamaah.js). Purely additive; does NOT affect prayer marking or window gating)
...
- `notifications` — prayer-reminder push config. Shape: `{ prayer: { enabled, perPrayer: { Fajr, Dhuhr, Asr, Maghrib, Isha } }, fcmTokens[], timezone, prayerTimes: { date: "YYYY-MM-DD", times: { Fajr: "05:23", ... } }, lastSentAt: { "YYYY-MM-DD_Fajr": ISO, ... } }`. `prayer.perPrayer` gates the cron per prayer (it skips only `perPrayer[p] === false`) and is now surfaced in the RemindersPanel as five per-prayer on/off chips (all default on). **Split ownership:** the client writes ONLY its own sub-fields (`prayer`/`timezone`/`prayerTimes` via nested-map merge, `fcmTokens` via `arrayUnion` of the *added* token only — see `pickClientOwnedNotifications` + the delta-lane `updateNotifications`); `lastSentAt` and token *pruning* are server-owned (`api/notify-prayers.js` admin writes). The on-load token refresh goes through **`mergeRefreshedToken(prev, {token, timezone})`** (pure, in `lib/sync.js`): it arrayUnions a genuinely-new token AND rewrites `timezone` whenever it has drifted (e.g. the user travelled), returning the same ref when nothing changed. The prior path only wrote on a new token, so an established device kept a stale timezone and the cron fired its reminders at the wrong local wall-clock time. This stops the old client/server clobber where a client flush of the whole `notifications` object overwrote the server's freshly-pruned tokens / dedup stamps. `fcmTokens[]` is multi-device (each browser/PWA install gets its own); the server endpoint prunes tokens that FCM reports as unregistered. `prayerTimes` is written by `usePrayer` when the client fetches today's timings **and** by the server cron itself: `notify-prayers` now computes times from the user's stored location (`settings.prayerLat/prayerLng`, else `prayerCity/prayerCountry`) via Aladhan when the cached `prayerTimes` is missing/stale, and writes the result back (see `api/_prayer.js` for the pure helpers, and the per-tick fetch memo). **This makes reminders self-healing** — they no longer require the user to have opened the app today. `lastSentAt` is keyed by `${userLocalDate}_${prayer}` and is GC'd to today's keys on each successful push. Server still skips a user only if there's **neither** fresh times **nor** a resolvable location (can't compute) — better a missed reminder than a wrong-time push.
...
**Silent loss is now surfaced**, not hidden: `deriveConnBadge` (pure) drives a single header badge from `{loading, loaded, online, serverTimedOut, syncState}`. `online` tracks `navigator.onLine`; a **server watchdog** (`SERVER_WATCHDOG_MS`, ~8s) flips `serverTimedOut` when the app has rendered from cache but no SERVER snapshot arrived — the exact stalled-Listen-channel condition. The badge shows offline / "can't reach server — saved locally" / saving / not-saved, and the delta-lane copy is truthful because those fields persist to the offline queue even while `!loaded`. **The positive counterpart** is a brief header "✓ Saved" flash — a *transient* confirmation, NOT a persistent badge. Planner watches `syncState` and shows it for ~2.5s only on a real save completion (the `"saving" → "synced"` transition), gated by `!connBadge` so it never overlaps the alarm/"Saving…" states, then clears itself — reassurance the moment a change lands, without cluttering every page. (An earlier version anchored a relative "✓ Saved · <when>" label to *load time* with a 30s ticker; that counted upward forever — "32m ago" — reading as staleness rather than reassurance, so it was replaced with the save-anchored flash.) This is display-only; it never touches the write machinery.
...
The focus timer (`pomMode` = `"focus"` | `"break"`) runs off a single `setInterval` in a `useEffect`. When a focus session completes: appends to `focusLog` (the only write — task minute/session totals derive from focusLog, so nothing is written back onto the goal), plays `playTimerSound("focusEnd")` (ascending C-major arpeggio, repeats once after 1.7s). `elapsedRef` tracks partial focus time so `endFocusEarly` can credit a partial session. **A completed session never marks the task done** — `SessionBanner` offers a "Mark task done" prompt (a habit "logs today"), always a user choice. Per-task `eta` overrides `pomDurations.defaultFocus` and acts as a **draining budget** (dial starts at `eta − logged`): **per-DAY for habits** (today's logged minutes, so a habit's budget doesn't drain permanently), **lifetime for one-shots**. When the budget is spent, starting the task fires the `onBudgetSpent` confirm (mark-complete / log-for-today) instead of opening another block. Timer settings persist via `updateSettings({ ...userSettings, pomDurations })`. **Pre-warm the AudioContext** by calling `getAudioCtx()` synchronously inside Start click handlers — browsers block programmatic audio outside of user gestures.

**Prayer-aware nudge:** while a session is running, the Pomodoro view shows a gentle, dismissible "prayer soon" banner as the next prayer's **effective time** approaches (within `NUDGE_THRESHOLD_MINS`). Effective time = the user's optional jamā'ah time (`settings.jamaahTimes[prayer]`) if set, else the Aladhan start. Pure logic in [src/lib/jamaah.js](src/lib/jamaah.js) (`nextPrayerNudge` skips prayers already prayed and any past today; recomputes each tick since `pomSeconds` re-renders the view). It **never auto-stops** the timer — the user decides — and is dismissible per prayer. The jamā'ah times are edited in an optional collapsed card on the Prayer view.
...
- **Aladhan** (`api.aladhan.com/v1/timingsByCity` and `/v1/timings`) for prayer times by city or geolocation. The **calculation method + Asr madhab are user-configurable** (`settings.prayerMethod` / `settings.prayerSchool`, defaulting to `2`/`1` = ISNA + Hanafi Asr, so existing users are unchanged). The `method=…&school=…` fragment is built by `methodSchoolParam` — client copy in [src/lib/prayerConfig.js](src/lib/prayerConfig.js) (also exports `CALC_METHODS` / `ASR_SCHOOLS` for the Prayer-view picker), server copy in [api/_prayer.js](api/_prayer.js) (kept in lockstep by tests; `api/` stays self-contained). Both `usePrayer` and the `notify-prayers` cron read the user's setting; the cron's per-tick fetch memo keys on the method/school too, so users at the same location with different madhabs don't share a cached fetch. The Prayer view's **calculation-method picker is its own collapsed card** (not buried in the change-location form). Selected city is persisted to `settings.prayerCity` / `settings.prayerCountry` and re-fetched on load via `settingsAppliedRef`; while that restore is in flight `usePrayer` exposes `prayerRestoring` so the view shows a "restoring…" placeholder instead of flashing the empty location form, and a **failed** restore now surfaces a soft error rather than silently dropping the user on that form.
